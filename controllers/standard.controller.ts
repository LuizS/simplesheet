...
abstract class StandardController {
  ...
  constructor(
    ...
    cssFiles: string[],
    jsFiles: string[],
    viewModelConverter: StandardViewModelConverter = new StandardViewModelConverter(),
    repository: ICellRepository = new CellRepository(),
  ) {
  ...
  async saveChange(req: Request, res: Response) {
    ...
    await this.repository.save(
      this.viewModelConverter.convertToModel(
        cellViewModel.row,
        cellViewModel.column,
  ...
  async saveAll(req: Request, res: Response) {
    ...
    try {
      ...
      await this.repository.saveAll(
        this.viewModelConverter.convertToModels(data),
      )
      res.redirect('/')
    } catch (e) {
